...
jQuery(document).ready(function(jQuery) {
	var inpc = jQuery('input[id*=color]')
	jQuery.each(inpc, function(){
		var vehmakeID = this.id;
		jQuery(this).before("<div id=\"CS_"+vehmakeID+"\" class=\"color-selector\"><div></div>");
	});
	var primColor = jQuery("div#colorSelectorPrim").next("input").val();		
	jQuery("div.color-selector").each( function(){
		var nextInputVal = jQuery(this).next("input").val();
		$(this).next("input").css("color", nextInputVal);
		$(this).children().css("backgroundColor", nextInputVal);
	});
});
...
(function(jQuery){
	...
	var initLayout = function() {
		...
		jQuery.each(inpc, function(){
			var vehmakeID = this.id;
			jQuery('div#CS_'+vehmakeID+'').ColorPicker({
				color: '#0000ff',
				...
				onChange: function (hsb, hex, rgb) {
					jQuery("div#CS_"+vehmakeID+"").next("input").val('#' + hex);
					jQuery("div#CS_"+vehmakeID+" div").css("backgroundColor", '#' + hex);	
					jQuery("div#CS_"+vehmakeID+"").next("input").css("color", '#' + hex);	
